...
import com.zfl.entity.vo.UserSendAddressVo;
...
public class SuerSendAddressController {
    ...
    public Result getAddressList(HttpServletRequest request){
        Integer userId = userJwtService.getUserId(request);
        //根据用户Id查询用户地址
    ...
    //@RequiresAuthentication
    public Result addAddress (HttpServletRequest request, @RequestBody TbUserSendAddress tbUserSendAddress, @RequestParam String oper){
        tbUserSendAddress.setUserId(userJwtService.getUserId(request));
        //设置默认地址
        ...
        //判断为哪种操作 oper 0为添加、1为编辑、2为删除
         if(oper.equals("0")){// 0为添加
             tbUserSendAddress.validateParams();
     ...
     * @return void
     */
    public void setDefault(HttpServletRequest request,int id){
        Integer userId = userJwtService.getUserId(request);
        //根据用户Id查询用户地址
        List<TbUserSendAddress> addressList = tbUserSendAddressService.list(new QueryWrapper<TbUserSendAddress>().eq("user_id",userId));
        addressList.forEach(add->{
