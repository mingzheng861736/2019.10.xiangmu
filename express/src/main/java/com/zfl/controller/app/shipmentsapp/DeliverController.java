...
public class DeliverController {
    ...
    public Result userDeliver(HttpServletRequest request,@RequestBody Decliver decliver) throws Exception {
        //验证
        decliver.validateParams();
        //获取当前登录的用户
        System.out.println(userJwtService.getUser(request).toString());
        //生成系统订单
       String uuid = UUID.randomUUID().toString().replace("-", "").trim().substring(2,20);
        String sysOrderNum = "KDDD-"+uuid;

        //先查询此用户单笔快递价格
        double orderPrice = 0.01;
        TbUserSingle userSingle = tbUserSingleService.getOne(new QueryWrapper<TbUserSingle>().eq("user_id", userJwtService.getUserId(request)));
        ...
        //设置对象
        setOddNum(tbOddNum,decliver);
        tbOddNum.setUserId(userJwtService.getUserId(request));
        ...
        TbArea area = tbAreaService.getById(decliver.getOrderArea());
        tbOddNum.setPickUpPerson(area.getAgencyId());
        ...
        if (save){
            ...
            if (StringUtils.equals(decliver.getPayType(),"1")){
                ...
                //微信支付
                Result result = wxPayService.pay(request,sysOrderNum);
                return result;
    ...
    public Result getAllOrder(HttpServletRequest request,@RequestBody QueryVo queryVo){
        List<TbOddNum> list = tbOddNumService.getOrderByCondition(request,queryVo);
        list.forEach(o->{
            //根据userId查询用户详情
     ...
     */
    //@RequiresAuthentication
    @PostMapping("goOnPayTheOrder")
    public Result goOnPayTheOrder(HttpServletRequest request,@RequestParam Integer id,String payType) throws Exception {
        TbOddNum tbOddNum = tbOddNumService.getById(id);
        //获取订单状态
        if(tbOddNum.getOrderState()!=0){
            return new Result(201,"此订单已支付或已过期,请确认！","");
        }
        //支付宝
        AliPayRespones aliPayRespones = new AliPayRespones();
        if (StringUtils.equals(payType,"1")){
            aliPayRespones = aliPayService.getPreOrder(tbOddNum.getGoodInfo(),tbOddNum.getPayPrice(),tbOddNum.getSysNum());
            String form=aliPayRespones.getPayUrl().replace("<script>document.forms[0].submit();</script>","");
            System.out.println(form);
            return new Result(200,"请求成功",form);
        }else{
            //微信支付
            Result result = wxPayService.pay(request,tbOddNum.getSysNum());
            return result;
        }
     ...
     */
    private void setOddNum(TbOddNum tbOddNum,Decliver decliver ){
    ...
    public Result cancelOrder(@RequestParam Integer id){
        ...
        //做收益回收
        //首先判断此订单状态
        if(tbOddNum.getOrderState()>5){//订单状态不为零则说明已经付款
