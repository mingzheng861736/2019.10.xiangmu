...
public class TbUserInfoController {
    ...
    public Result getUserInfoById (HttpServletRequest request){
        ...
        myProfile.put("userType",adminUser.getUserType());//用户类型
        myProfile.put("orderArea",tbAreaService.getById(adminUser.getUserArea()));//用户订单所在区域
    ...
    @Transactional
    public Result changeUserInfo(HttpServletRequest request,@RequestBody Map<String,Object> data){
        Integer userId = userJwtService.getUserId(request);
